...
typedef struct __mavlink_camera_status_t {
 uint64_t time_usec; /*< [us] Image timestamp (microseconds since UNIX epoch, according to camera clock)*/
 ...
 *
 * @param time_usec [us] Image timestamp (microseconds since UNIX epoch, according to camera clock)
 ...
 *
 * @return [us] Image timestamp (microseconds since UNIX epoch, according to camera clock)
 */
